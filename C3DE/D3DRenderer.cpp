...
void D3DRenderer::GenTriGrid(int numVertRows, int numVertCols,
				float dx, float dz, 
...
void D3DRenderer::buildGeoBuffers()
{
	std::vector<D3DXVECTOR3> verts;
	std::vector<DWORD> indices;
	...
	HR(m_device->CreateVertexBuffer(mNumVertices * sizeof(VertexPos), 
	...
	HR(m_device->CreateIndexBuffer(mNumTriangles*3*sizeof(WORD), D3DUSAGE_WRITEONLY,
...
void D3DRenderer::DrawMesh(Mesh *a_mesh)
{
	...
	HR(m_device->SetStreamSource(0, mVB, 0, sizeof(VertexPos)));
	...
	HR(m_device->DrawIndexedPrimitive(D3DPT_TRIANGLELIST, 0, 0, mNumVertices, 0, mNumTriangles));
