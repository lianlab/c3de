...
FireRingParticleSystem::FireRingParticleSystem(IDirect3DTexture9 * a_texture,int a_maxNumParticles, float a_timePerParticle, D3DXVECTOR3 a_acceleration):
ParticleSystem(a_acceleration,a_maxNumParticles, a_timePerParticle)
{
	...
	m_effect = ShaderManager::GetInstance()->GetFXByID(SHADER_FIRE_RING_FX_ID);
	...
	FireRingFX *t_effect = (FireRingFX*) m_effect;
	t_effect->SetViewportHeight(clientRect.bottom);

}

FireRingParticleSystem::~FireRingParticleSystem()
{

}

void FireRingParticleSystem::InitParticle(VertexParticle& out)
{
...
void FireRingParticleSystem::SetShaderHandlers()
{
	FireRingFX *t_effect = (FireRingFX*) m_effect;
