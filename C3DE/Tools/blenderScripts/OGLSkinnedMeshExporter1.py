...
#*********************************************************************************************************************************************
	...
	def writeMeshcoordArm(self, obj ,arm_ob, objectIndex):
		...
		path = ("C:\documents and Settings\csabino\Desktop\exportedMeshes\out%d_ogl.c3d" % objectIndex)
		path2 = ("C:\documents and Settings\csabino\Desktop\exportedMeshes\out_ogl.txt")
		path3 = ("C:\documents and Settings\csabino\Desktop\exportedMeshes\out2_ogl.txt")
		...
		#format = "iffffffff"                   # one integer
		#formatTest = "iiifff"                   # one integer		
		#dataTest = struct.pack(formatTest, 25, 13, 69, 132.0, 15.5, 19.3)			
		#fileBin.write(dataTest)
		...
		t_coordIterator = 0
		t_indexIterator = 0
		print("total vertices %i" % len(new_vert))
		#file2.write("float [] textureBuffer = new float[%i];\n" % (len(new_vert) * 2))
		file2.write("float [] textureBuffer = getTextureBuffer();\n")
		#file2.write("float [] vertexBuffer = new float[%i];\n" % (len(new_vert) * 3))
		file2.write("float [] vertexBuffer = getVertexBuffer();\n")
		#file2.write("short[] indexBuffer = new short[%i];\n\n" % (len(new_vert)))
		file2.write("short[] indexBuffer = getIndexBuffer();\n\n")
		
		file2.write("int [] verticesBone0 = new int[%i];\n" % len(indices))
		file2.write("int [] verticesBone1 = new int[%i];\n" % len(indices))
		file2.write("float [] verticesBone0Weights = new float[%i];\n\n" % len(indices))
		
		textureBufferStr = "float textureBuffer[] = new float[%i];\n" % (len(new_vert) * 2)
		vertexBufferStr = "float vertexBuffer[] = new float[%i];\n" % (len(new_vert) * 3)
		indexBufferStr = "short indexBuffer[] = new short[%i];\n" % (len(new_vert))
		...
		path4 = ("C:/documents and Settings/csabino/Desktop/exportedMeshes/test_mesh.bin")
		fileBin = open(path4, "wb")		
		formatBin = "i"
		dataBin = struct.pack(formatBin, (len(new_vert)))	
		fileBin.write(dataBin)
		dataBin = struct.pack(formatBin, (len(indices)))
		fileBin.write(dataBin)
		#format = "iffffffff"                   # one integer
		#formatTest = "iiifff"                   # one integer		
		#dataTest = struct.pack(formatTest, 25, 13, 69, 132.0, 15.5, 19.3)			
		...
		formatBin = "fff"
		for vv in new_vert:
			dataBin = struct.pack(formatBin, vv[0], vv[1], vv[2])	
			fileBin.write(dataBin)
		...
		vert_iterator = 0
		for vv in new_vert:
			dataBin = struct.pack(formatBin, new_vert_uvs[vert_iterator][0], 1.0 - new_vert_uvs[vert_iterator][1])	
			fileBin.write(dataBin)
			vert_iterator += 1
		...
		formatBin = "i"
		for ii in indices:
			dataBin = struct.pack(formatBin, ii)	
			fileBin.write(dataBin)
			
		#vertices bone 0 indices
		formatBin = "i"
		vert_iterator = 0
		for vv in new_vert:
			dataBin = struct.pack(formatBin, g_bone_indices[vert_iterator][0])	
			fileBin.write(dataBin)
			vert_iterator += 1
		
		vert_iterator = 0	
		
		#vertices bone 1 indices
		formatBin = "i"
		vert_iterator = 0
		for vv in new_vert:
			dataBin = struct.pack(formatBin, g_bone_indices[vert_iterator][1])	
			fileBin.write(dataBin)
			vert_iterator += 1
		
		vert_iterator = 0	
		
		#vertices bone 0 weights
		formatBin = "f"		
		for vv in new_vert:
			dataBin = struct.pack(formatBin, 1.0)	
			fileBin.write(dataBin)
		...
		for vv in new_vert:
			...
			if hasTexture:				
				...
				#file2.write("textureBuffer[%i]= %ff;\n"	% (t_uvIterator, new_vert_uvs[vert_iterator][0]));
				textureBufferStr += "textureBuffer[%i]= %ff;\n"	% (t_uvIterator, new_vert_uvs[vert_iterator][0])
				t_uvIterator = t_uvIterator + 1
				#file2.write("textureBuffer[%i]= %ff;\n"	% (t_uvIterator, 1.0 - new_vert_uvs[vert_iterator][1]));
				textureBufferStr += "textureBuffer[%i]= %ff;\n"	% (t_uvIterator, 1.0 - new_vert_uvs[vert_iterator][1])
				t_uvIterator = t_uvIterator + 1
		...
		for vv in new_vert:
			...
			if hasTexture:				
				#file2.write("vertexBuffer[%i]= %ff;\n"	% (t_coordIterator, vv[0] ));
				vertexBufferStr += "vertexBuffer[%i]= %ff;\n"	% (t_coordIterator, vv[0] )
				t_coordIterator = t_coordIterator + 1
				#file2.write("vertexBuffer[%i]= %ff;\n"	% (t_coordIterator, vv[1] ));
				vertexBufferStr += "vertexBuffer[%i]= %ff;\n"	% (t_coordIterator, vv[1] )
				t_coordIterator = t_coordIterator + 1
				#file2.write("vertexBuffer[%i]= %ff;\n"	% (t_coordIterator, vv[2] ));
				vertexBufferStr += "vertexBuffer[%i]= %ff;\n"	% (t_coordIterator, vv[2] )
				t_coordIterator = t_coordIterator + 1
		...
		file2.write("verticesBone0 = getVerticesBone0();\n")
		file2.write("verticesBone1 = getVerticesBone1();\n")
		file2.write("verticesBone0Weights = getVerticesBone0Weights();\n\n")
			
		vert_iterator = 0
		
		verticesBones0Str = ""
		verticesBones1Str = ""
		verticesBones0WeightsStr = ""
		
		for vv in new_vert:
			
				
			verticesBones0Str += ("\tverticesBone0[%i] = %i;\n" % (vert_iterator, g_bone_indices[vert_iterator][0]))
			verticesBones1Str += ("\tverticesBone1[%i] = %i;\n" % (vert_iterator, g_bone_indices[vert_iterator][1]))		
			verticesBones0WeightsStr += ("\tverticesBone0Weights[%i] = %i;\n" % (vert_iterator, 1.0))		
			
			"""
			file2.write("verticesBone0[%i] = %i;\n" % (vert_iterator, g_bone_indices[vert_iterator][0]))
			file2.write("verticesBone1[%i] = %i;\n" % (vert_iterator, g_bone_indices[vert_iterator][1]))
			file2.write("verticesBone0Weights[%i] = %i;\n\n" % (vert_iterator, 1.0))
			"""
		...
		print("total indices %i" % len(indices))
		for ii in indices:
			
			#file2.write("indexBuffer[%i]= %i;\n" % (ii, ii))
			indexBufferStr += "indexBuffer[%i]= %i;\n" % (ii, ii)
		...
		file2.write("\nm_testTexture = new DC3DTexture();\n")
		file2.write("\nResourceManager.setTexture(m_testTexture, TextureIDs.RID_GFX_TEXTURE_TEST_MESH);\n")
		file2.write("\nm_testMesh = new DC3DSkinnedMesh();\n")
		file2.write("\nint vertexCount = %i;\nint indexCount = %i;\n" % (len(indices), len(indices)))
		
		file2.write("\n\n\n")
		file3.write("private int[] getVerticesBone0(){\n\tint [] verticesBone0 = new int[%i];\n\n%s\n\treturn verticesBone0;\n}\n" % (len(new_vert),verticesBones0Str))
		file3.write("private int[] getVerticesBone1(){\n\tint [] verticesBone1 = new int[%i];\n\n%s\n\treturn verticesBone1;\n}\n" % (len(new_vert),verticesBones1Str))
		file3.write("private float[] getVerticesBone0Weights(){\n\tfloat [] verticesBone0Weights = new float[%i];\n\n%s\n\treturn verticesBone0Weights;\n}\n" % (len(new_vert),verticesBones0WeightsStr))
		file3.write("private float[] getTextureBuffer(){\n\n%s\n\treturn textureBuffer;\n}\n" % (textureBufferStr))
		file3.write("private float[] getVertexBuffer(){\n\n%s\n\treturn vertexBuffer;\n}\n" % (vertexBufferStr))
		file3.write("private short[] getIndexBuffer(){\n\n%s\n\treturn indexBuffer;\n}\n" % (indexBufferStr))
