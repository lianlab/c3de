...
{
	//D3DMesh *d3d_mesh = (D3DMesh *) mesh;
	//HR(m_device->SetStreamSource(0, d3d_mesh->GetVertexBuffer(), 0, d3d_mesh->GetVertexSize()));
	HR(m_device->SetStreamSource(0, m_vb, 0, sizeof(VertexPos)));
	//HR(m_device->SetIndices(d3d_mesh->GetIndexBuffer()));
	HR(m_device->SetIndices(m_ib));
	HR(m_device->SetVertexDeclaration(VertexPos::Decl));
	//HR(m_device->SetVertexDeclaration(d3d_mesh->GetVertexDeclaration()));

	D3DXMATRIX W;
	D3DXMatrixIdentity(&W);
	HR(m_device->SetTransform(D3DTS_WORLD, &W));	
	D3DXMATRIX mt = ((D3DCamera*)m_camera)->GetMatrix();

	HR(m_device->SetTransform(D3DTS_VIEW, &mt));
